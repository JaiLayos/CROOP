...
public class Activity_Edit_User_Profile extends AppCompatActivity {
        ...
        edit.setOnClickListener(v -> {
            ...
            docRef.update(
                    ...
                    "Updated At", new Date()
                        ...
                        Intent intent = new Intent(this, Sign_In_Success_Customer.class);
    ...
    private void sendToPG(Customer customer) {
        UserAPI userAPI = RetrofitClient.getClient().create(UserAPI.class);
        Call<Customer> byFirebaseID = userAPI.getCustomerByFirebaseID(user.getUid());
        byFirebaseID.enqueue(new Callback<Customer>() {
            @Override
            public void onResponse(Call<Customer> call, Response<Customer> response) {
                Customer gotCustomer = response.body();
                Call<Customer> update = userAPI.updateCustomer(gotCustomer.getId(), customer);
                update.enqueue(new Callback<Customer>() {
                    @Override
                    public void onResponse(Call<Customer> call, Response<Customer> response) {
                        if (response.isSuccessful()) {
                            Toast.makeText(Activity_Edit_User_Profile.this, "Na-update na ang user sa PostgreSQL!", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(Activity_Edit_User_Profile.this, "Hindi na-update ang profile: " + response.message(), Toast.LENGTH_SHORT).show();
                        }
                    }

                    @Override
                    public void onFailure(Call<Customer> call, Throwable t) {
                        Toast.makeText(Activity_Edit_User_Profile.this, "Nagkaproblema: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
            }
