...
public class Activity_Discount_Inventory_Individual extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.discount_individual_seller);
    ...
    private void initializeComponents() {
        ...
        Call<List<IndividualSellersDiscount>> call = apiService.getIndividualDiscountbyFirebaseID(user.getUid());
        call.enqueue(new Callback<List<IndividualSellersDiscount>>() {
            @Override
            public void onResponse(Call<List<IndividualSellersDiscount>> call, Response<List<IndividualSellersDiscount>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    try {
                        ...
                        Toast.makeText(Activity_Discount_Inventory_Individual.this, "API_ERROR: Code " + response.code(), Toast.LENGTH_SHORT).show();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            @Override
            public void onFailure(Call<List<IndividualSellersDiscount>> call, Throwable t) {
                Toast.makeText(Activity_Discount_Inventory_Individual.this, "API_ERROR"+ t.toString(), Toast.LENGTH_SHORT).show();
                t.printStackTrace();
            }
        });
        Button find, add;
        ...
        find.setOnClickListener(v -> {
            ...
            Call<List<IndividualSellersDiscount>> searchItem = apiService.getIndividualDiscountByName(itemName);
            searchItem.enqueue(new Callback<List<IndividualSellersDiscount>>() {
                @Override
                public void onResponse(Call<List<IndividualSellersDiscount>> call, Response<List<IndividualSellersDiscount>> response) {
                    if (response.isSuccessful() && response.body() != null) {
        ...
        back.setOnClickListener(v -> {
            Intent intent = new Intent(this, Sign_In_Success_Individual_Seller.class);
    ...
    private void populateTableDefault(List<IndividualSellersDiscount> items, TableLayout table) {
        UserAPI apiService = RetrofitClient.getClient().create(UserAPI.class);
        table.removeViews(1, table.getChildCount() - 1);

        for (IndividualSellersDiscount item : items) {
