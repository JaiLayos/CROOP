...
public class Activity_Item_Inventory extends AppCompatActivity {
    ...
    private void initializeComponents() {
        ...
        call.enqueue(new Callback<List<GroupSellersItemInventory>>() {
            ...
            public void onResponse(Call<List<GroupSellersItemInventory>> call, Response<List<GroupSellersItemInventory>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    try {
                        ...
                        Toast.makeText(Activity_Item_Inventory.this, "API_ERROR: Code " + response.code(), Toast.LENGTH_SHORT).show();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
            ...
            public void onFailure(Call<List<GroupSellersItemInventory>> call, Throwable t) {
                Toast.makeText(Activity_Item_Inventory.this, "API_ERROR"+ t.toString(), Toast.LENGTH_SHORT).show();
                t.printStackTrace();
            }
        });
        Button find, add;
    ...
    private void populateTableDefault(List<GroupSellersItemInventory> items, TableLayout table){
        ...
        for (GroupSellersItemInventory item : items) {
            ...
            itemTextView.setOnClickListener(view -> {
                BottomSheetDialog bottomSheetDialog = new BottomSheetDialog(Activity_Item_Inventory.this);
                ...
                delete.setOnClickListener(v -> {
                    Call<GroupSellersItemInventory> deleteItem = apiService.deleteItem(item.getId());
                ...
                update.setOnClickListener(v -> {
                    ...
                    try {
                        ...
                        groupSellersItemInventory.setItemName(itemNameText);
                        groupSellersItemInventory.setItemStart(item.getItemStart());
                        groupSellersItemInventory.setItemUsed(itemUsedValue);

                        Call<GroupSellersItemInventory> updateItem = apiService.updateItem(item.getId(), groupSellersItemInventory);
                        updateItem.enqueue(new Callback<GroupSellersItemInventory>() {
                            @Override
                            public void onResponse(Call<GroupSellersItemInventory> call, Response<GroupSellersItemInventory> response) {
                                if (response.isSuccessful() && response.body() != null) {
